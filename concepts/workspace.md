...
**State that churns stays out of the versioned tree.** Once automation lives in the town (scheduled sweeps, session logging, watchers), it produces state on every run: current findings, per-run logs, poll results. Versioning that alongside conventions and plans makes the workspace repository permanently dirty and buries the changes that matter. Separate the two kinds. *Transient* state is regenerated each run and can be ignored by version control. *Durable* state (what a sweep learned last time, decisions a human made) is versioned, either in the workspace repository or in a side store with its own history. Make the split by location so it is decided once, not per file: a `state/` directory per automation with an ignored transient half and a versioned durable half, or a classifier that sorts paths into "safe to commit unattended", "wait for a human", and "not ours". For a worked example, see [State That Churns](../examples/pickletown/beyond-the-spec.md#state-that-churns) in the Pickletown companion.
